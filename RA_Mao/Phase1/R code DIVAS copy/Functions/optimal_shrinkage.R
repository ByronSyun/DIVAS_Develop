optimal_shrinkage <- function(singvals, beta, loss, percentile = 0.5, sigma = NULL) {
  stopifnot(length(beta) == 1, beta <= 1, beta > 0)
...
optshrink_impl <- function(singvals, beta, loss, sigma) {
  stopifnot(sigma > 0, length(sigma) == 1)
  
  x <- function(y) {
    sqrt(0.5 * ((y^2 - beta - 1) + sqrt((y^2 - beta - 1)^2 - 4 * beta))) *
      (y >= 1 + sqrt(beta))
  }
  ...
  switch(
    loss,
    'fro' = { singvals <- sigma * opt_fro_shrink(singvals / sigma) },
    'nuc' = {
      y <- singvals / sigma
      singvals <- sigma * opt_nuc_shrink(y)
      singvals[x(y)^4 - sqrt(beta) * x(y) * y - beta <= 0] <- 0
    },
    'op' = { singvals <- sigma * opt_op_shrink(singvals / sigma) },
    stop("Unknown loss type")
  )
...
incMarPas <- function(x0, beta, gamma) {
  ...
  return(integrate(fun, x0, topSpec)$value)
}
